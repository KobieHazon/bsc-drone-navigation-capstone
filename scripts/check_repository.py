...
for forbidden in ROOT.rglob("*"):
    ...
    if any(part.startswith("._") for part in forbidden.parts):
        ...
        sys.exit(1)
    if forbidden.suffix == ".pyc":
        print(f"Generated bytecode: {rel}", file=sys.stderr)
        sys.exit(1)
    if forbidden.name in {"all of us.png", "itzchak.jpeg", "marzim.png"}:
        print(f"People image should not be staged: {rel}", file=sys.stderr)
        sys.exit(1)

text_files = [p for p in ROOT.rglob("*") if p.is_file() and ".git" not in p.parts and p.suffix.lower() in {".py", ".md", ".csv", ".txt", ".drawio", ""}]
